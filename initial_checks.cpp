...
//    char *base_name = basename(command_line_file);
    ...
    if (strcmp(base_name, "body.bin") == 0)
    {
        ...
        {
            qsnprintf(g_target_guid, temp_len + 1, "%s", temp);
        }
    }
    else
    {
        ...
        if (g_target_guid != NULL)
        {
            qsnprintf(g_target_guid, temp_len + 1, "%s", base_name);
        }
    }
    ...
    if (g_config.generate_stats == 1)
    {
        ...
        if (g_config.output_log == 1)
        {
            ...
            qsnprintf(output_name, sizeof(output_name), "%s/log", "C:\\");

            FILE *output_file = qfopen(output_name, "w+");
            if (output_file == NULL)
            {
                ERROR_MSG("Can't open log file: %s %s.", output_name, "C:\\");
                return;
            }
 ...
 * via the offsets
 */
static int locate_bootservices_table(ea_t start_addr, ea_t end_addr, ea_t *out_addr)
{
    ...
    {
        insn_t cmd;
        decode_insn(&cmd,current_addr);
        /* we are looking for a mov instruction that involves the RDX register and Boot Services offset in System table */
        if (cmd.itype == NN_mov && cmd.ops[1].type == o_displ && cmd.ops[1].phrase == 0x2)
        {
            if (cmd.ops[0].type == o_reg && cmd.ops[1].addr == 0x60)
            {
                boot_register = cmd.ops[0].reg;
                found_boot = 1;
            }
        }
        ...
        if (found_boot)
        {
            if (cmd.itype == NN_mov && cmd.ops[1].type == o_reg && cmd.ops[1].reg == boot_register && cmd.ops[0].type == o_mem)
            {
                DEBUG_MSG("Found boot storage at 0x%llx Type: %x Address: 0x%llx", current_addr, cmd.ops[0].type, cmd.ops[0].addr);
                set_name(cmd.ops[0].addr, "BootServices_table", SN_CHECK);
                *out_addr = cmd.ops[0].addr;
                break;
            }
        }
 ...
 * via the offsets
 */
static int locate_runtimeservices_table(ea_t start_addr, ea_t end_addr, ea_t *out_addr)
{
    ...
    do
    {
        ...
        if (cmd.itype == NN_mov && cmd.ops[1].type == o_displ && cmd.ops[1].phrase == 0x2)
        {
            if (cmd.ops[0].type == o_reg && cmd.ops[1].addr == 0x58)
            {
                runtime_register = cmd.ops[0].reg;
                found_runtime = 1;
            }
        }
        if (found_runtime)
        {
            if (cmd.itype == NN_mov && cmd.ops[1].type == o_reg && cmd.ops[1].reg == runtime_register && cmd.ops[0].type == o_mem)
            {
                DEBUG_MSG("Found runtime storage at 0x%llx", current_addr);
                set_name(cmd.ops[0].addr, "RunTimeServices_table", SN_CHECK);
                *out_addr = cmd.ops[0].addr;
                break;
            }
        }
...
static int find_system_tables(void)
{
    ...
    for (int idx = 0; idx < get_func_qty(); idx++)
    {
        ...
        if (strcmp(fname.c_str(), "start") == 0 || strcmp(fname.c_str(), "_ModuleEntryPoint") == 0)
        {
            DEBUG_MSG("Found function %s at %llx", fname, f->start_ea);
            found = 1;
            break;
        }
    }
    ...
    int ret = 0;
    ret = locate_bootservices_table(f->start_ea, f->end_ea, &bootservices_ptr);
    /* try alternative method - there might be a call processing it */
    if (ret != 0)
    {
        DEBUG_MSG("Trying to locate boot services inside functions");
        ea_t current_addr = f->start_ea;
        while ((current_addr = find_code(current_addr, SEARCH_DOWN)) != BADADDR && current_addr <= f->end_ea)
        {
            insn_t cmd;
            decode_insn(&cmd, current_addr);
            ...
            {
                func_t *call_f = NULL;
                call_f = get_func(cmd.ops[0].addr);
                if (call_f == NULL)
                {
                    continue;
                }
                /* try to locate the boot table inside, stop if successful */
                if (locate_bootservices_table(call_f->start_ea, call_f->end_ea, &bootservices_ptr) == 0)
                {
                    break;
                }
            }
    ...
    ret = locate_runtimeservices_table(f->start_ea, f->end_ea, &runtimeservices_ptr);
    if (ret != 0)
    {
        ...
        while ((current_addr = find_code(current_addr, SEARCH_DOWN)) != BADADDR && current_addr <= f->end_ea)
        {
            ...
            if (cmd.itype == NN_call) // XXX: call types? near? far?
            {
                ...
                if (locate_runtimeservices_table(call_f->start_ea, call_f->end_ea, &runtimeservices_ptr) == 0)
                {
                    break;
                }
            }
    ...
    LL_FOREACH(g_boot_services_stats.analysis_head, entry)
    {
        ...
        while ((current_addr = find_code(current_addr, SEARCH_UP)) != BADADDR && max_nr_insts > 0)
        {
            ...
            if (entry->type == kLocateProtocol)
            {
                ...
                if (cmd.itype == NN_lea && cmd.ops[0].type == o_reg && cmd.ops[0].reg == REG_RCX)
                {
                    ea_t target_guid_addr = cmd.ops[1].addr;
                    /* retrieve the GUID being used and store in the structure field */
//                    get_bytes(target_guid_addr, (void*)&current_guid, sizeof(EFI_GUID));
                    get_bytes((void*)&current_guid, sizeof(EFI_GUID), target_guid_addr);
            ...
            else if (entry->type == kHandleProtocol)
            {
                ...
                if (cmd.itype == NN_lea && cmd.ops[0].type == o_reg && cmd.ops[0].reg == REG_RDX)
                {
                    ea_t target_guid_addr = cmd.ops[1].addr;
                    /* retrieve the GUID being used and store in the structure field */
                    get_bytes((void*)&current_guid, sizeof(EFI_GUID), target_guid_addr);
...
static int locate_boot_services_refs(void)
{
    ...
    if (bootservices_ptr != 0)
    {
        ...
        for ( bool ok=xb.first_to(bootservices_ptr, XREF_ALL); ok; ok=xb.next_to() )
        {
            ...
            insn_t cmd;
            decode_insn(&cmd,ref_to_table);
            /* find the type of reference */
            if (cmd.itype == NN_mov)
            {
                /* loading the boot table pointer into a register */
                if (cmd.ops[1].type == o_mem && cmd.ops[1].addr == bootservices_ptr)
                {
                    type = kLoad;
                    boot_dst_reg = cmd.ops[0].reg;
                }
                /* storing the boot table pointer */
                else if (cmd.ops[0].type == o_mem && cmd.ops[0].addr == bootservices_ptr)
                {
                    type = kStore;
                    boot_src_reg = cmd.ops[1].reg;
                }
            }
            if (type == kInvalid)
            {
            ...
            while ((current_addr = find_code(current_addr, SEARCH_DOWN)) != BADADDR && current_addr <= function_end)
            {
                ...
                if (type == kStore && cmd.ops[0].type == o_reg && cmd.ops[0].reg == boot_src_reg)
                {
                    break;
                }
                if ((cmd.itype == NN_callni  || cmd.itype == NN_jmpni) && cmd.ops[0].type == o_displ)
                {
                    if ((type == kStore && cmd.ops[0].phrase == boot_src_reg) ||
                        (type == kLoad && cmd.ops[0].phrase == boot_dst_reg))
                    {
                        struct service_refs *new_ref_entry = (struct service_refs*)malloc(sizeof(struct service_refs));
                        if (new_ref_entry != NULL)
                        {
                            new_ref_entry->offset = cmd.ops[0].addr;
...
static int locate_runtime_services_refs(void)
{
    ...
    if (runtimeservices_ptr != 0)
    {
        ...
        for ( bool ok=xb.first_to(runtimeservices_ptr, XREF_ALL); ok; ok=xb.next_to() )
        {
            ...
            {
                /* loading the boot table pointer into a register */
                if (cmd.ops[1].type == o_mem && cmd.ops[1].addr == runtimeservices_ptr)
                {
                    type = kLoad;
                    runtime_dst_reg = cmd.ops[0].reg;
                }
                /* storing the boot table pointer */
                else if (cmd.ops[0].type == o_mem && cmd.ops[0].addr == runtimeservices_ptr)
                {
                    type = kStore;
                    runtime_src_reg = cmd.ops[1].reg;
                }
            }
            if (type == kInvalid)
            {
            ...
            while ((current_addr = find_code(current_addr, SEARCH_DOWN)) != BADADDR && current_addr <= function_end)
            {
                ...
                decode_insn(&cmd,current_addr);
                /* verify if register was modified before the call so we give up in that case */
                if (type == kStore && cmd.ops[0].type == o_reg && cmd.ops[0].reg == runtime_src_reg)
                {
                    break;
                }
                if ((cmd.itype == NN_callni || cmd.itype == NN_jmpni) && cmd.ops[0].type == o_displ)
                {
                    if ((type == kStore && cmd.ops[0].phrase == runtime_src_reg) ||
                        (type == kLoad && cmd.ops[0].phrase == runtime_dst_reg))
                    {
                        struct service_refs *new_ref_entry = (struct service_refs*)malloc(sizeof(struct service_refs));
                        if (new_ref_entry != NULL)
                        {
 ...
 */
static void add_boot_analysis_entry(ea_t address, enum system_services type)
{
...
static int find_data_seg_guids(void)
{
    ...
    ea_t current_addr = seg_info->start_ea;
    
    while (current_addr != BADADDR && current_addr <= seg_info->end_ea)
    {
        EFI_GUID guid = {0};
        
//        get_bytes(current_addr, (void*)&guid, sizeof(EFI_GUID));
        get_bytes((void*)&guid, sizeof(EFI_GUID),current_addr);
...
static void sql_file_entry(void)
{
    ...
    sqlite3_bind_text(sqlStatement, 2, "c:\\toto.log", -1, SQLITE_STATIC);
    /* XXX: fix type */
